...
class MatrixService {
    ...
    public function findFreeSpace($area)
    {
        for ($i = 0; $i)
    }
    
    public function findSpawnPosition($grid)
    {
        $potentialSpace = array($grid); // PROVISOIRE!!!!
        $freeSpace = findFreeSpace($potentialSpace); // PROVISOIRE!!!!
        $posX = $freeSpace[0];
        $posY = $freeSpace[1];
    }
